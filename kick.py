    ...
    if sq["type"] in ("gamble", "chest"):
        ...
        if effect == "pay_each":
            ...
            for uid in balances.keys():
                ...
                balances[uid] += 10
                tribute_lines.append(f"<@{uid}> ✅ <:coin:1418612412885635206> + $10 → 💰 ${balances[uid]} ")
                total_tribute += 10
            ...
            embed = discord.Embed(
                ...
                description=(
                        ...
                        f"(<:coin:1418612412885635206> $10 to each free matey)\n\n"
                        f"💸 Tribute Results:\n" + "\n".join(tribute_lines) +
...
async def collect_slot_owner_tax():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for g in guilds:
            ...
            log_lines = [
                f"📑 <@{uid}>: <:coin:1418612412885635206> -**${total:,.2f}** fee | 💳 **Balance: <:coin:1418612412885635206> ${bal:,.2f}**"
                for uid, count, base, idle, total, bal in tax_details
            ]
...
async def before_tax_loop():
    ...
    next_target = next_target.replace(hour=2, minute=18, second=0, microsecond=0)
